...
if (@$_POST['acao']=="filtrar"){
    $professoresz= lista_administradores($_POST['filtro']);
} else {
    $professoresz=lista_administradores("");
}
?>
<!doctype html>
<html lang="en">
                        ...
                        <div class="my-3 p-3 bg-white rounded shadow-sm">
                            <?php 
                             if ((@$_POST['acao']=="filtrar") and (@$_POST['filtro']!=="") ){
                                ...
                                $i=0;
                                if(!@$professoresz['size-professores']){
                                    echo "<br /><i>Não foram encontrados resultados para a busca <b>'".@$_POST['filtro']."'</b></i>";
                                    $professoresz['size-professores']=0;
                                }
                                while($i<$professoresz['size-professores']){?>
                                    <div class="media text-muted pt-3"  id="">
                                        <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                            <div class="d-flex justify-content-between align-items-center w-100">
                                                <strong class="text-gray-dark"><?php echo ucfirst($professoresz['professores'][$i]['nome']);?></strong>
                                                <form class="form" action="" method="post">
                                                    <input type="hidden" id="acao1" name="acao1" value="deletar"> 
                                                    <input type="hidden" id="professores-id" name="professores-id" value="<?=$professoresz['professores'][$i]['id']?>"> 
                                                    <?php 
                                                    if (@$_POST['acao']=="filtrar"){?>
                                                    <input type="hidden" id="acao1" name="acao" value="filtrar"> 
                                            ...
                                            <span class="d-block"><?=$professoresz['professores'][$i]['cpf']?></span>
                                            <span class="d-block"><?=$professoresz['professores'][$i]['email']?></span>
                                            <!-- <span class="d-block"><?php echo ucfirst($professoresz['professores'][$i]['status']);?></span> -->
                                        </div>  
                                    </div>
                                <?php 
                                $i++;
