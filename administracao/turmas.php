...
if (@$_POST['acao1'] == "deletar") {
    deletar_turma_id($_POST['turmas-id']);
}
if (@$_POST['acao'] == "filtrar") {
    $turmas = lista_turmas($_POST['filtro']);
} else {
    $turmas = lista_turmas("");
}
?>
                    ...
                    <div>
                        ...
                        <div class="row text-justify">
                            <div class="col- col-sm-4 col-md-3 col-lg-2 pb-2 pt-1">
                                <button type="button" class="btn btn-primary btn-sm btn-block" onclick="window.location.href='/administracao/turmas-novo'">+ Adicionar</button>
                            </div>
                        ...
                        <div class="my-3 p-3 bg-white rounded shadow-sm">
                            ...
                            if ((@$_POST['acao'] == "filtrar") and (@$_POST['filtro'] !== "")) {
                                echo '<a href="/administracao/turmas" style="color: #009933;"class="text-succes">Mostrar todos as turmas</a><br/>';
                            }
                            $i = 0;
                            if (!@$turmas['size-turmas']) {
                                echo "<br /><i>Não foram encontrados resultados para a busca <b>'" . @$_POST['filtro'] . "'</b></i>";
                                $turmas['size-turmas'] = 0;
                            }
                                ...
                                <div class="media text-muted pt-3" id="">
                                    <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                        <div class="d-flex justify-content-between align-items-center w-100">
                                            <div>
                                                <?php $periodoNum = periodo_num($turmas['turmas'][$i]['id']);?>
                                                <form class="form" action="/administracao/turmas-editar" method="post">
                                                    <input type="hidden" id="acao13" name="acao1" value="editar">
                                                    <input type="hidden" id="turmas-id3" name="id" value="<?= $turmas['turmas'][$i]['id'] ?>">
                                                    <input type="hidden" id="turmas-id" name="cursos" value="<?= $periodoNum['periodo']['curso'] ?>">
                                                    <input calss='pt-1 mr-1' type="image" src="/sx/common/assets/icon-editar.png" alt="Submit" width="22px">
                                                </form>
                                                <strong class="text-gray-dark"><?php echo ucfirst($turmas['turmas'][$i]['nome']); ?></strong><br>
                                                <?php $periodo = lista_de_disciplinas_turma($turmas['turmas'][$i]['id']);
                                                ...
                                                echo '<br><strong class="text-gray-dark">Disciplinas:</strong>';
                                                if ($periodo['resposta'] != false) {
                                                    while ($i2 < 6) {
                                                        $nome = busca_nome_disciplina($periodo['disci'][$i2]);
                                                        echo $nome['nome'] . '|';
                                                        $i2++;
                                                    }
                                                }
                                        ...
                                        <span class="d-block">Código da turma: <?= $turmas['turmas'][$i]['id'] ?></span>
                                        <!-- <span class="d-block"><?php echo ucfirst($turmas['turmas'][$i]['status']); ?></span> -->
                                    </div>
                                </div>
                            <?php
                                $i++;
