...
if (@$_POST['acao1'] == "deletar") {
    deletar_aluno_id($_POST['alunos-id']);
}
if (@$_POST['acao'] == "filtrar") {
    $alunos = lista_alunos($_POST['filtro']);
} else {
    $alunos = lista_alunos("");
}
?>
                    ...
                    <div>
                        ...
                        <div class="row text-justify">
                            <div class="col- col-sm-4 col-md-3 col-lg-2 pb-2 pt-1">
                                <button type="button" class="btn btn-primary btn-sm btn-block" onclick="window.location.href='/administracao/alunos-novo'">+ Adicionar</button>
                            </div>
                        ...
                        <div class="my-3 p-3 bg-white rounded shadow-sm">
                            ...
                            if ((@$_POST['acao'] == "filtrar") and (@$_POST['filtro'] !== "")) {
                                echo '<a href="/administracao/alunos" style="color: #009933;"class="text-succes">Mostrar todos as alunos</a><br/>';
                            }
                            $i = 0;
                            if (!@$alunos['size-alunos']) {
                                echo "<br /><i>Não foram encontrados resultados para a busca <b>'" . @$_POST['filtro'] . "'</b></i>";
                                $alunos['size-alunos'] = 0;
                            }
                            while ($i < $alunos['size-alunos']) { ?>
                                <div class="media text-muted pt-3" id="">
                                    <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                        <div class="d-flex justify-content-between align-items-center w-100">
                                            <div>
                                                <form class="form" action="/administracao/alunos-editar" method="post">
                                                    <input type="hidden" id="acao13" name="acao1" value="editar">
                                                    <input type="hidden" id="alunos-id3" name="id" value="<?= $alunos['alunos'][$i]['id'] ?>">
                                                    <input calss='pt-1 mr-1' type="image" src="/sx/common/assets/icon-editar.png" alt="Submit" width="22px">
                                                </form>
                                                <strong class="text-gray-dark"><?php echo ucfirst($alunos['alunos'][$i]['nome']); ?></strong>
                                            </div>
                                            <div>
                                                <form class="form" action="" method="post">
                                                    <input type="hidden" id="acao1" name="acao1" value="deletar">
                                                    <input type="hidden" id="alunos-id" name="alunos-id" value="<?= $alunos['alunos'][$i]['id'] ?>">
                                                    <?php
                                                    if (@$_POST['acao'] == "filtrar") { ?>
                                                        <input type="hidden" id="acao1" name="acao" value="filtrar">
                                        ...
                                        <span class="d-block">Matícula: <?= $alunos['alunos'][$i]['matricula'] ?></span>
                                        <span class="d-block">Código da aluno: <?= $alunos['alunos'][$i]['id'] ?></span>

                                        <?php
                                        if (isset($alunos['alunos'][$i]['turma_id'])) {
                                            $turma = pegar_turmas_aluno($alunos['alunos'][$i]['turma_id']);
                                        ?>
                                            <span class="d-block">Turma: <?= $turma['nome'] ?></span>
                                        <?php } ?>
                                         <?php
                                                                        if ($alunos['alunos'][$i]['turma_id']!=0) {
                                                                            $periodo = periodo_num($alunos['alunos'][$i]['turma_id']);
