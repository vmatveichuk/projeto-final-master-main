...
$activity = "administracao-administradores";
...
if (@$_POST['acao1'] == "deletar") {
    deletar_admin_id($_POST['admin-id']);
}
if (@$_POST['acao'] == "filtrar") {
    $administradores = lista_administradores($_POST['filtro']);
} else {
    $administradores = lista_administradores("");
}
?>
<!doctype html>
<html lang="en">
                    ...
                    <div>
                        ...
                        <div class="row text-justify">
                            <div class="col- col-sm-4 col-md-3 col-lg-2 pb-2 pt-1">
                                <button type="button" class="btn btn-primary btn-sm btn-block" onclick="window.location.href='/administracao/administradores-novo'">+ Adicionar</button>
                            </div>
                        ...
                        <div class="my-3 p-3 bg-white rounded shadow-sm">
                            ...
                            if ((@$_POST['acao'] == "filtrar") and (@$_POST['filtro'] !== "")) {
                                echo '<a href="/administracao/administradores" style="color: #009933;"class="text-succes">Mostrar todos os adminstradores</a><br/>';
                            }
                            $i = 0;
                            if (!@$administradores['size-administradores']) {
                                echo "<br /><i>Não foram encontrados resultados para a busca <b>'" . @$_POST['filtro'] . "'</b></i>";
                                $administradores['size-administradores'] = 0;
                            }
                            while ($i < $administradores['size-administradores']) { ?>
                                <div class="media text-muted pt-3" id="">
                                    <div class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                                        <div class="d-flex justify-content-between align-items-center w-100">
                                            <strong class="text-gray-dark"><?php echo ucfirst($administradores['administradores'][$i]['nome']); ?></strong>
                                            <form class="form" action="" method="post">
                                                <input type="hidden" id="acao1" name="acao1" value="deletar">
                                                <input type="hidden" id="admin-id" name="admin-id" value="<?= $administradores['administradores'][$i]['id'] ?>">
                                                <?php
                                                if (@$_POST['acao'] == "filtrar") { ?>
                                                    <input type="hidden" id="acao1" name="acao" value="filtrar">
                                        ...
                                        <span class="d-block"><?= $administradores['administradores'][$i]['cpf'] ?></span>
                                        <span class="d-block"><?= $administradores['administradores'][$i]['email'] ?></span>
                                        <!-- <span class="d-block"><?php echo ucfirst($administradores['administradores'][$i]['status']); ?></span> -->
                                    </div>
                                </div>
                            <?php
                                $i++;
