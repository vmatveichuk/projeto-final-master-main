...
function curl_request2($data=false, $url='auth.academus.pro/auth_api/login.php') {
    // Tratamento inicial dos dados recebidos
...
if(!isset($_POST['email'])){
    ...
    exit;
        ...
        if($resposta != NULL and $email != "" && $email == $resposta->user->email){
            ...
            $data['cpf'] = $_POST['cpf'];
            $response = curl_request1($data, 'auth.academus.pro/auth_api/update_password.php');
            senha_mail_2($email, $senha_unadultered);
            //sendemail_senha_email($email, $senha_unadultered);
            
            $data['senha'] = $senha;

            $resposta2 = curl_request2($data, 'auth.academus.pro/auth_api/recover-password.php');
            //var_dump($resposta2);
            //exit;
